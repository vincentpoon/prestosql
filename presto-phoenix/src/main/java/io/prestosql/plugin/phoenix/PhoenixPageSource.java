...
import static io.prestosql.plugin.phoenix.MetadataUtil.isPrimaryKeyColumn;
        ...
        implements ConnectorPageSource
{
            ...
            try {
                for (int i = 0; i < ROWS_PER_REQUEST; i++) {
                    ...
                    for (int column = 0; column < columnTypes.size(); column++) {
                        ...
                        else if (isPrimaryKeyColumn(columnName, columnType)) {
                            writePrimaryKeyColumns(output, (RowType) columnType);
                        }
                        else {
    ...
    private void writePrimaryKeyColumns(BlockBuilder output, RowType rowType)
            throws SQLException
    {
    ...
    private PrestoException closeWithSuppression(Throwable throwable)
    {
        try {
            close();
        }
            ...
            if (throwable != closeException) {
                throwable.addSuppressed(closeException);
            }
        }
        return new PrestoException(PHOENIX_ERROR, throwable);
    }
